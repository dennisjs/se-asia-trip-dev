...
window.initMapWithPhotos = function () {
  ...
  fetchLatestLocation().then(locations => {
    ...
    // Add gray markers and info boxes for previous locations
    locations.slice(1).forEach((loc, i) => {
      if (loc.lat && loc.lng) {
        ...
        const next = locations[i] ? new Date(locations[i].arrival_date) : null;
        ...
        if (next) {
          const departStr = new Date(next.getTime() - 86400000).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
